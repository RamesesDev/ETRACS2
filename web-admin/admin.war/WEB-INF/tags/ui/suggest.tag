	...
	<tr>
		...
		<td>
			<input class="form-input" type="text" r:context="${context}" r:name="${objPrefix}${name}" r:caption="${caption}"
				...
				<c:if test="${! empty suggestName}"> r:suggestName="${suggestName}" </c:if>
				<c:if test="${! empty suggestExpression}"> r:suggestExpression="${suggestExpression}" </c:if>
				<c:if test="${! empty suggest}"> r:suggest="${suggest}" </c:if>
			/>
		</td> 
	</tr>
</c:if>
